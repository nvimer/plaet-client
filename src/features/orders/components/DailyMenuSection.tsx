import { Soup, GlassWater, Leaf, CircleDot, Sparkles, Check } from "lucide-react";
  ...
  return (
    <div 
      className={cn(
        ...
        hasOptions ? colorClasses[color] : "bg-carbon-100 border-carbon-200 text-carbon-500"
      )}
        ...
        <div className="space-y-2">
          {options.length === 1 ? (
            ...
            <div className="p-3 bg-white rounded-lg border-2 border-current shadow-sm">
              <span className="text-base font-bold block">{options[0].name}</span>
            </div>
          ) : (
    ...
    <Card className={cn("overflow-hidden shadow-lg", className)}>
      ...
      <div className="px-4 py-3 bg-sage-50 border-t border-sage-100">
        <p className="text-xs text-carbon-500 text-center">
          El cliente puede elegir una opción de cada categoría marcada como &quot;Requerido&quot;
        </p>
      </div>
    </Card>
  );
